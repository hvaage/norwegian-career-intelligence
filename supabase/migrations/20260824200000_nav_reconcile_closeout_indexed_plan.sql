-- The closeout candidate query is a bounded anti-join. Force indexed probes
-- for this function so PostgreSQL does not scan the entire reconciliation
-- snapshot before honouring the batch limit.
...
BEGIN
  -- Each candidate needs one lookup in the run-specific active snapshot.
  -- A merge/hash anti-join scans the whole snapshot before LIMIT applies.
  ...
  WITH modern AS MATERIALIZED (
    ...
    UPDATE public.job_opportunities j
    SET status = 'INACTIVE', source_payload_hash = c.inactive_payload_hash,
        last_reconciled_run_id = p_run_id, last_reconciled_at = now(),
