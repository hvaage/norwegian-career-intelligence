-- Force one unconditional ?last bootstrap so the Edge Function can persist
-- NAV's concrete response feed_url. Opportunity data is not touched.
UPDATE public.nav_feed_sync_state
SET
  feed_url = '/api/v1/feed?last',
  last_next_url = '/api/v1/feed?last',
  feed_etag = NULL,
  feed_last_modified = NULL,
  tail_reached_at = NULL,
...
AS $$
  WITH inventory AS (
    SELECT
      (SELECT count(*) FROM public.job_opportunities WHERE source = 'nav') AS total,
      (SELECT count(*) FROM public.job_opportunities WHERE source = 'nav' AND status = 'ACTIVE') AS active,
      (SELECT count(*) FROM public.job_opportunities WHERE source = 'nav' AND status = 'INACTIVE') AS inactive,
      (
