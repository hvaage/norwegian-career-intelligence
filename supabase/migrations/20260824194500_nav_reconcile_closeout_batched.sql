-- Keep NAV closeout below the database statement timeout. A run remains in
-- `closing` until no eligible active NAV row is absent from its snapshot.

CREATE INDEX IF NOT EXISTS idx_job_opportunities_nav_active_closeout
  ON public.job_opportunities (source_event_version, external_id)
  WHERE source = 'nav' AND status = 'ACTIVE';

CREATE INDEX IF NOT EXISTS idx_job_opportunities_nav_active_missing_version
  ON public.job_opportunities (external_id)
  WHERE source = 'nav' AND status = 'ACTIVE' AND source_event_version IS NULL;
...
BEGIN
  ...
  WITH candidates AS MATERIALIZED (
    SELECT
      ...
      COALESCE(j.source_event_version, public._nav_compute_event_version_row(j)) AS source_event_version,
